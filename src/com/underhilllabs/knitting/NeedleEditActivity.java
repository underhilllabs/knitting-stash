...
public class NeedleEditActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		ArrayAdapter<CharSequence> adapter_length = ArrayAdapter
				.createFromResource(this, R.array.length_array,
						android.R.layout.simple_spinner_item);
		adapter_length
				.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
		...
		add_button.setOnClickListener(new Button.OnClickListener() {
			public void onClick(View v) {
				...
				ndb.updateNeedle(rowid, size, size_i, type, material, length,
						length_i, is_metric, is_crochet, notes, in_use);
				Intent i = new Intent(NeedleEditActivity.this,
						KnittingStashHome.class);
				i.putExtra("com.underhilllabs.knitting.tabid", 0);
				startActivity(i);
				// Toast.makeText(NeedleEditActivity.this,
				// "Added to Needle Inventory", Toast.LENGTH_SHORT).show();
	...
	private boolean setWidgets(long row_id) {
		...
		if (cur.getString(cur.getColumnIndex(DbAdapter.KEY_TYPE)).equals(
				"circular")) {
			radio_circular.setChecked(true);
			spinner_length.setAdapter(adapter_length_circ);
		} else if (cur.getString(cur.getColumnIndex(DbAdapter.KEY_TYPE))
				.equals("dpns")) {
		...
		// Toast.makeText(NeedleEditActivity.this, " 1:"+cur.getString(6) ,
